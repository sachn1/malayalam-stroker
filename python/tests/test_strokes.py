...
def test_produces_at_least_one_glyph():
    trace = shape_word("നന്ദി", FONT)
    assert len(trace["glyphs"]) >= 1


@pytest.mark.parametrize("word", CONJUNCT_WORDS)
def test_conjuncts_collapse_to_fewer_glyphs_than_codepoints(word):
...
def test_glyphs_ordered_left_to_right():
    trace = shape_word("മലയാളം", FONT)
