...
def main() -> None:
    """Read stroke-data.json, smooth every stroke, write stroke-data-snapped.json.
...
# How many arc-length-uniform points to sample from each raw stroke
N_SAMPLES = 120

# RDP epsilon — higher = more aggressive simplification (fewer bezier segments)
# In font units (~1000 UPM).  Try 8–20; lower = more faithful to hand shape.
RDP_EPSILON = 20.0

# Bezier segments in the output path (controls output smoothness vs file size)
N_OUT = 50
...
def sample_path(d: str, n: int) -> np.ndarray:
    """Sample n arc-length-uniform points from an SVG path. Returns (n, 2)."""
    try:
        path = svgpathtools.parse_path(d)
    except Exception:
        return np.empty((0, 2))
...
def _rdp_indices(pts: np.ndarray, epsilon: float) -> list[int]:
    """Return indices of the simplified polyline (RDP algorithm)."""
    if len(pts) < 3:
        return list(range(len(pts)))
    start, end = pts[0], pts[-1]
    seg = end - start
    ...
    if seg_len < 1e-9:
        ...
        # Perpendicular distance from each point to the start-end line (2D cross product)
        d = pts - start
        dists = np.abs(seg[0] * d[:, 1] - seg[1] * d[:, 0]) / seg_len
    idx = int(np.argmax(dists))
    if dists[idx] > epsilon:
        left  = _rdp_indices(pts[:idx + 1], epsilon)
        right = _rdp_indices(pts[idx:], epsilon)
...
def rdp(pts: np.ndarray, epsilon: float) -> np.ndarray:
    """Reduce *pts* using the Ramer-Douglas-Peucker algorithm."""
    indices = _rdp_indices(pts, epsilon)
    return pts[indices]
...
def smooth_stroke(stroke_d: str) -> str:
    """
    Fit a smooth cubic bezier spline to the hand stroke.
    Preserves start/end, overall shape, and stroke direction.
    Returns a new SVG path string with clean C1-continuous bezier segments.
    """
    pts = sample_path(stroke_d, N_SAMPLES)
    if len(pts) < 4:
        return stroke_d

    # Simplify to key waypoints — removes mouse-wobble noise
    waypts = rdp(pts, RDP_EPSILON)
    if len(waypts) < 4:
        waypts = pts  # fall back to all samples if RDP over-simplifies

    x, y = waypts[:, 0], waypts[:, 1]

    # Avoid duplicate consecutive points (splprep requires distinct knots)
    keep = np.ones(len(x), dtype=bool)
    for i in range(1, len(x)):
        if abs(x[i] - x[i-1]) < 0.1 and abs(y[i] - y[i-1]) < 0.1:
            keep[i] = False
    ...
    try:
        # s=0 → interpolate through the simplified waypoints exactly
        tck, _ = splprep([x, y], s=0, k=3)
    except Exception:
        return stroke_d

    # Evaluate spline + derivative at N_OUT uniform parameter values
    u_new = np.linspace(0, 1, N_OUT)
    xs, ys         = splev(u_new, tck)
    dxs, dys       = splev(u_new, tck, der=1)

    # Build C1-continuous cubic bezier path using Hermite → Bezier conversion.
    # For segment i → i+1:
    #   CP1 = P[i]  + d[i]  / 3 * dt
    #   CP2 = P[i+1]- d[i+1]/ 3 * dt
    dt = 1.0 / (N_OUT - 1)
    parts = [f"M {xs[0]:.1f} {ys[0]:.1f}"]
    for i in range(N_OUT - 1):
        cp1x = xs[i]   + dxs[i]   * dt / 3
...
def main() -> None:
    ...
    for cluster, entry in stroke_data.items():
        ...
        for i, stroke in enumerate(entry.get("strokes", [])):
            original_d = stroke["d"]
            smoothed_d = smooth_stroke(original_d)
            smoothed_strokes.append({"d": smoothed_d})
            status = "smoothed" if smoothed_d != original_d else "unchanged (too short)"
            print(f"  {cluster!r} stroke {i+1}: {status}")
        out[cluster] = {"strokes": smoothed_strokes}

    OUT_PATH.write_text(json.dumps(out, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"\nWritten {OUT_PATH.relative_to(ROOT)}")
    print("Drop it onto the demo to compare against the originals.")
